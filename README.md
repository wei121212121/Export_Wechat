...
模型压缩包位于 GitHub 的发布文件中：`ExportWeChat-中文语音模型-v1.zip`。

下载并解压后，将其中的 `models` 文件夹放到程序目录旁边，然后使用 `启动内测版.cmd` 启动程序。模型只用于语音转文字，不包含微信聊天记录、账号信息或授权令牌。

模型文件较大，仓库使用 Git LFS 存储；首次下载前请安装 Git LFS 并执行 `git lfs pull`。
